...
内核要找到一些用来存放程序指令和数据的空闲内存页。内核还要建立数据结构来存放相应的内存分配情况和进程属性
...
fork出的子进程从父进程fork后部分开始执行代码

可以使用fork返回的pid区分当前是父进程还是子进程在执行，父进程fork后返回子进程pid，子进程在父进程fork子进程代码段的fork返回0给子进程
...
exec只是替换了进程的二进制代码和数据，文件描述符表属于进程属性，不会因为执行exec替换代码和数据而改变，因此在执行exec前打开的文件和文件描述符指向的文件都不会发生变化
