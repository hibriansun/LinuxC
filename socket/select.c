 ...
 * 1. 如果采用 非阻塞 socket，对所有socket进行轮番查询状态（有数据到达读取数据，没有则下一个）   ----   这种方法虽然简单，但是低效
 * 2. 如果让系统监视socket并 阻塞 他们，当有socket上发生事件时，系统通知服务器进程哪个socket上发生什么事件，服务器进程再去处理   --  高效许多，避免无事件发生时CPU轮番查询非阻塞socket
